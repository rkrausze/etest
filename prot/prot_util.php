...
function makeorderhead_tables(&$tables, $orderinfo) {
    $info = explode(',', $orderinfo.",,,,,"); // Anchor, column, direction, type.
    for ($i = 0; $i < count($tables); $i++) {
...
define('ETEST_EXCELROWS', 65535);
define('ETEST_FIRSTUSEDEXCELROW', 3);
...
function print_tables_xls($filenameprefix, $tables) {
    ...
    $nropages = 0;
    foreach ($tables as $table) {
        $nropages += ceil(count($table->data) / (ETEST_EXCELROWS - ETEST_FIRSTUSEDEXCELROW + 1));
    }
    ...
    $wsnumber = 1;
    foreach ($tables as $table) {
        $npagetable = ceil(count($table->data) / (ETEST_EXCELROWS - ETEST_FIRSTUSEDEXCELROW + 1));
        for ($j = 1; $j <= $npagetable; $j++) {
            ...
            foreach ($table->head as $item) {
                $worksheet[$wsnumber]->write(ETEST_FIRSTUSEDEXCELROW - 1, $col, $item, $formatbold);
                $col++;
            }
            $wsnumber++;
        }
    ...
    $row = ETEST_FIRSTUSEDEXCELROW;
    $wsnumber = 1;
    $myxls =& $worksheet[$wsnumber];
    foreach ($tables as $table) {
        foreach ($table->data as $line) {
            if ($row > ETEST_EXCELROWS) {
                $wsnumber++;
                $myxls =& $worksheet[$wsnumber];
                $row = ETEST_FIRSTUSEDEXCELROW;
            }
